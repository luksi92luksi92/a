...
class Observation:
    ...
    confidence: float
    provenance: str
    stem_distribution: List[float] = field(default_factory=list)  # optional HTDemucs cue, empty if unavailable
...
@dataclass
class StyleVector:
    """Continuous, multidimensional behavior vector (Sec 16: 'Instead
    represent style as a combination of observable behavior... Subgenres
    are regions/clusters in a multidimensional style space, not
    independent universes.'). THIS vector -- not any genre label -- is the
    real, always-available output. Built entirely from state already
    computed elsewhere (groove, timbre distribution, role distribution,
    pattern confidence, dynamics), never from a per-genre classifier."""
    tempo_bpm: float = 0.0
    ...
    timing_tightness: float = 0.0    # 1 - normalized microtiming deviation
    pct_percussive: float = 0.0      # fraction of live objects with PERCUSSIVE timbre
    pct_sustained: float = 0.0       # fraction of live objects with SUSTAINED timbre
    mean_kick_role: float = 0.0
    mean_bass_role: float = 0.0
    mean_percussion_role: float = 0.0
    mean_harmonic_pad_role: float = 0.0
    mean_lead_melody_role: float = 0.0
    pattern_confidence: float = 0.0
    mean_crest_factor: float = 0.0
    object_density: float = 0.0
    last_updated: float = 0.0

    def as_vector(self) -> np.ndarray:
        """Normalized feature vector for distance/similarity comparisons."""
        return np.array([
            min(self.tempo_bpm / 180.0, 1.5), self.syncopation, self.swing, self.regularity,
            min(self.event_density / 8.0, 1.0), self.timing_tightness, self.pct_percussive,
            self.pct_sustained, self.mean_kick_role, self.mean_bass_role, self.mean_percussion_role,
            self.mean_harmonic_pad_role, self.mean_lead_melody_role, self.pattern_confidence,
            min(self.mean_crest_factor / 6.0, 1.0), min(self.object_density / 10.0, 1.0),
        ])


@dataclass
class StyleRegion:
    """A single hand-specified, explicitly ILLUSTRATIVE reference point in
    style space (Sec 16 hierarchy example). NOT derived from any labeled
    dataset -- this system has none, by design (Sec 17/Dependency Policy:
    no large external corpora). A track landing far from every region is
    exactly as valid an outcome as one landing close to one -- 'A new/
    unrecognized subgenre should still be interpretable through its
    behavior vector' even with zero reference match."""
    name: str
    parent: Optional[str]
...
class TimbreFingerprint:
    """Pitch-normalized spectral-envelope descriptor for cross-object
    'have I heard this specific timbre before, recently, in this track'
    matching (Sec 17, lightweight version). Low-order cepstral coefficients
    (log bark-band energy -> DCT, the classical MFCC construction using the
    bark filterbank already built for masking) capture spectral-ENVELOPE
    shape while discarding fine harmonic/pitch detail -- exactly the
    'pitch-normalized' property needed to recognize the same instrument at
    a different pitch. NOT a trained embedding: no dataset, no training
    step, just a running average of a classical DSP feature.

    Honest limitation: sampled from the object's currently-matched hop's
    FULL spectrum, not a perfectly isolated single-source spectrum -- in a
    dense polyphonic moment it partially reflects whatever else is
    sounding too. Only sampling on hops where this object was confidently
    matched (not every hop) mitigates but doesn't eliminate this."""
...
class SoundObject:
    ...
    timbre_fingerprint: TimbreFingerprint = field(default_factory=TimbreFingerprint)
...
def beat_alignment(groove: "GrooveVector", event_time: float) -> float:
    """0 = event_time landed exactly on a beat, 1 = exactly off-beat.
    Shared by SemanticRoleEngine and PredictionModel (Sec 10.4 'rhythmic
    recurrence'). Falls back to a neutral 0.5 when no rhythmic context is
    available yet, rather than silently asserting misalignment."""
    if groove.tempo_confidence <= 0.2 or groove.tempo_bpm <= 0 or not groove.next_beat_time:
        return 0.5
    period_s = 60.0 / groove.tempo_bpm
    k = math.ceil((groove.next_beat_time - event_time) / period_s)
    grid_origin = groove.next_beat_time - period_s * k
    phase = ((event_time - grid_origin) / period_s) % 1.0
...
class WorldState:
    ...
    def __init__(self, params: Parameters):
        ...
        self.style: StyleVector = StyleVector()
        self.style_history: List[StyleVector] = []
    ...
    # ---- style (Sec 16): continuous multidimensional state, not a genre label -----------------
    def update_style(self, new_style: StyleVector, t: float) -> None:
        old = self.style
        self.update_log.append(UpdateRecord(t, "StyleEngine", "world.style", "tempo_bpm",
                                             old.tempo_bpm, new_style.tempo_bpm,
                                             "recomputed style vector from observable behavior",
                                             new_style.tempo_confidence))
        self.style = new_style
        self.style_history.append(new_style)
        if len(self.style_history) > 500:  # bounded: this only grows every UPDATE_INTERVAL_S
            self.style_history = self.style_history[-500:]
...
class AudioSource:
    ...
    def synth_test_signal(self, duration: float = 8.0, bpm: float = 120.0) -> np.ndarray:
        ...
        for i in range(1, n_hh, 2):  # odd i only: t = 0.25, 0.75, 1.25... -- genuinely off the downbeat
            s = int(i * hh_step * sr)
...
class PhysicalAnalyzer:
    ...
    def bark_energy_only(self, audio: np.ndarray) -> np.ndarray:
        """Per-hop bark-band energy on the SAME short-window/fast_hop grid
        as analyze_full, without the onset/pitch machinery -- used for
        separated stems, where only bark-band energy is needed for the
        stem-affinity cue (Sec 17)."""
        mag_short, _win, _raw = self._framed_spectrogram(audio, self.short_window,
                                                           self.p.short_fft_size, self.p.fast_hop)
        power_short = mag_short.astype(np.float64) ** 2
        return (power_short @ self.bark_filterbank.T).astype(np.float32)  # (n_hops, n_bark)
...
class ObservationGenerator:
    ...
    def generate(self, ev: PhysicalEvidence,
                 stem_bark_energy: Optional[Dict[str, np.ndarray]] = None) -> List[Observation]:
        ...
        if ev.is_transient:
            ...
            obs.append(Observation(
                ...
                stem_distribution=self._stem_distribution_at(band, ev.hop_index, stem_bark_energy)))
        ...
        for b in peak_bands:
            ...
            obs.append(Observation(
                ...
                stem_distribution=self._stem_distribution_at(b, ev.hop_index, stem_bark_energy)))
        return obs

    @staticmethod
    def _stem_distribution_at(band: int, hop_index: int,
                               stem_bark_energy: Optional[Dict[str, np.ndarray]]) -> List[float]:
        """Normalized [drums, bass, other, vocals, ...]-order energy share
        at this band/hop across separated stems, or [] if unavailable --
        empty means the stem-affinity bonus is provably a no-op for this
        observation (Sec 17: auxiliary evidence, never authoritative)."""
        if not stem_bark_energy:
            return []
        values = []
        for arr in stem_bark_energy.values():
            if hop_index < arr.shape[0]:
                values.append(max(float(arr[hop_index, band]), 0.0))
            else:
                values.append(0.0)
        total = sum(values)
        if total <= 1e-9:
...
class ObjectTracker:
    ...
    def __init__(self, params: Parameters, world: WorldState, prediction_model: "PredictionModel"):
        self.p = params
        self.world = world
    ...
    STEM_AFFINITY_MAX_BONUS = 0.10  # DERIVED: auxiliary evidence nudges, never overrides (Sec 17)

    def _stem_affinity_bonus(self, obs: Observation, obj: SoundObject) -> float:
        """Small additive bonus/malus from the optional HTDemucs stem cue.
        Provably 0.0 whenever stem data is unavailable on either side --
        the 6-cue mix-domain score above is completely unaffected when
        HTDemucs isn't installed or hasn't separated this file."""
        obs_dist = obs.stem_distribution
        obj_dist = obj.physical_signature.stem_distribution
        if not obs_dist or not obj_dist or len(obs_dist) != len(obj_dist):
            return 0.0
        a, b = np.array(obs_dist), np.array(obj_dist)
        na, nb = float(np.linalg.norm(a)), float(np.linalg.norm(b))
        if na < 1e-9 or nb < 1e-9:
            return 0.0
        cos_sim = float(np.dot(a, b) / (na * nb))  # -1..1, but both are non-negative distributions so 0..1
        return self.STEM_AFFINITY_MAX_BONUS * (cos_sim * 2.0 - 1.0)
    ...
    def step(self, observations: List[Observation], t: float) -> None:
        ...
        for obs in observations:
            for obj in candidate_objs:
                ...
                score += self._stem_affinity_bonus(obs, obj)
    ...
    def _birth(self, obs: Observation, t: float) -> SoundObject:
        ...
        sig = PhysicalSignature(frequency_hz=obs.frequency_hz, stable_frequency_hz=obs.frequency_hz,
                                 ...
                                 stem_distribution=list(obs.stem_distribution))
    ...
    def _apply_match(self, obj: SoundObject, obs: Observation, score: float,
        ...
        self.world.apply_update(UpdateRecord(t, "ObjectTracker", obj.object_id,
                                              ...
                                              pitch_conf))
        if obs.stem_distribution:
            self.world.apply_update(UpdateRecord(t, "ObjectTracker", obj.object_id,
                                                  "physical_signature.stem_distribution",
                                                  obj.physical_signature.stem_distribution,
                                                  list(obs.stem_distribution),
                                                  "stem-affinity snapshot at matched hop (HTDemucs cue)", score))
    ...
    def _update_prediction(self, obj: SoundObject, t: float) -> None:
        obj.prediction = self.prediction_model.predict(obj, t)
...
class GroupingEngine:
    ...
    SPLIT_GAP_MS = 600.0                # DERIVED: absence duration (not hop-count) before a member
                                         # splits off -- a layer should tolerate at least as much
                                         # per-hop matching noise as an individual object's own
                                         # HIDDEN/RECOVERING hysteresis already does, not less
    ...
    def __init__(self, params: Parameters, world: WorldState):
        ...
        self._member_last_fired: Dict[Tuple[str, str], float] = {}
    ...
    def update(self, t: float) -> None:
        ...
        # --- split checking: time-based absence, not a raw per-hop miss count ---
        for layer in list(self.world.layers.values()):
            if layer.status != "CONFIRMED":
                continue
            for m in layer.member_ids:
                if m in just_matched_ids:
                    self._member_last_fired[(layer.layer_id, m)] = t
            fired = [m for m in layer.member_ids if m in just_matched_ids]
            if not fired:
                continue  # layer had no activity at all this hop -- nothing to judge yet
            for m in list(layer.member_ids):
                skey = (layer.layer_id, m)
                last_fired = self._member_last_fired.get(skey, layer.formed_time)
                gap_ms = (t - last_fired) * 1000.0
                if gap_ms >= self.SPLIT_GAP_MS:
                    self.world.remove_layer_member(layer.layer_id, m, t,
                        reason=f"absent for {gap_ms:.0f}ms while the rest of the layer stayed active "
                                f"(>= split gap {self.SPLIT_GAP_MS:.0f}ms)")
                    self._member_last_fired.pop(skey, None)
...
class RhythmEngine:
    ...
    def __init__(self, params: Parameters, world: WorldState):
        ...
        self._onset_bass_weights: List[float] = []
        self._last_update_t = -1e9
        self._phase_origin: Optional[float] = None

    def observe_onset(self, t: float, strength: float, bass_weight: float = 0.3) -> None:
        self._onset_times.append(t)
        self._onset_strengths.append(max(float(strength), 0.0))
        self._onset_bass_weights.append(float(np.clip(bass_weight, 0.0, 1.0)))
        ...
        while self._onset_times and self._onset_times[0] < cutoff:
            ...
            self._onset_bass_weights.pop(0)
    ...
    def update(self, t: float) -> bool:
        ...
        if self._phase_origin is None:
            weighted = [os * (0.3 + 0.7 * bw) for os, bw in
                        zip(self._onset_strengths, self._onset_bass_weights)]
            best_idx = int(np.argmax(weighted))
            self._phase_origin = self._onset_times[best_idx]
        else:
            candidates = []
            for ot, os, bw in zip(self._onset_times, self._onset_strengths, self._onset_bass_weights):
                if (t - ot) >= period_s * 2:
                    continue
                candidates.append((ot, os * (0.3 + 0.7 * bw)))
            if candidates:
                latest = max(candidates, key=lambda c: c[1])[0]
...
class SemanticRoleEngine:
    ...
    def _score(self, obj: SoundObject, t: float, register_lo: float, register_hi: float,
        ...
        on_beat = 1.0 - beat_alignment(self.world.groove, obj.last_observed_time)
...
# =============================================================================
# 15. STYLE ENGINE — genre/subgenre/style as continuous space (Sec 16)
# =============================================================================

class StyleEngine:
    """Style-space characterization. Explicitly NOT a per-genre classifier
    (Sec 16: 'Do not build: one classifier per genre'). Builds a continuous
    StyleVector from behavior already computed elsewhere -- rhythm,
    microtiming, groove, timbre-distribution ('sound-design tendencies'),
    role distribution, pattern confidence, dynamics ('production
    characteristics'). Optionally reports similarity to a small set of
    hand-specified, explicitly illustrative reference regions -- 'Genre is
    probabilistic and hierarchical... Subgenres are regions/clusters in a
    multidimensional style space, not independent universes.'

    The reference regions are placeholders from general genre knowledge,
    NOT fit to any labeled dataset (none exists in this system, by design).
    A track resembling none of them is not a failure -- the vector itself
    is the real output ('A new/unrecognized subgenre should still be
    interpretable through its behavior vector').

    Runs on the slowest cadence in the system: style is the most
    aggregated, most slowly-changing signal here.
    """

    UPDATE_INTERVAL_S = 2.0             # DERIVED: slowest cadence in the system
    REGION_SIMILARITY_SCALE = 3.0       # DERIVED: distance-to-similarity scale
    REGION_REPORT_THRESHOLD = 0.55      # DERIVED: below this, report 'no close region' honestly

    def __init__(self, params: Parameters, world: WorldState):
        self.p = params
        self.world = world
        self.logger = logging.getLogger("StyleEngine")
        self._last_update_t = -1e9
        self._prev_top_region: Optional[str] = None
        self.regions = self._build_illustrative_regions()

    @staticmethod
    def _build_illustrative_regions() -> List[StyleRegion]:
        def vec(**kw) -> np.ndarray:
            return StyleVector(**kw).as_vector()
        return [
            StyleRegion(
                name="Four-on-the-floor / driving electronic", parent="Electronic",
                reference_vector=vec(tempo_bpm=128, syncopation=0.15, swing=0.5, regularity=0.85,
                                      event_density=4.0, timing_tightness=0.85, pct_percussive=0.6,
                                      pct_sustained=0.2, mean_kick_role=0.7, mean_bass_role=0.5,
                                      mean_percussion_role=0.6, mean_harmonic_pad_role=0.2,
                                      mean_lead_melody_role=0.2, pattern_confidence=0.8,
                                      mean_crest_factor=3.0, object_density=6.0),
                description="steady kick-driven pulse, tight timing, low syncopation"),
            StyleRegion(
                name="Syncopated / breakbeat-leaning", parent="Electronic",
                reference_vector=vec(tempo_bpm=140, syncopation=0.55, swing=0.6, regularity=0.5,
                                      event_density=5.0, timing_tightness=0.6, pct_percussive=0.55,
                                      pct_sustained=0.2, mean_kick_role=0.5, mean_bass_role=0.5,
                                      mean_percussion_role=0.65, mean_harmonic_pad_role=0.2,
                                      mean_lead_melody_role=0.25, pattern_confidence=0.5,
                                      mean_crest_factor=4.0, object_density=6.0),
                description="off-grid percussion emphasis, swung/irregular timing"),
            StyleRegion(
                name="Ambient / downtempo-leaning", parent="Electronic",
                reference_vector=vec(tempo_bpm=80, syncopation=0.3, swing=0.5, regularity=0.4,
                                      event_density=1.0, timing_tightness=0.4, pct_percussive=0.15,
                                      pct_sustained=0.6, mean_kick_role=0.15, mean_bass_role=0.3,
                                      mean_percussion_role=0.15, mean_harmonic_pad_role=0.7,
                                      mean_lead_melody_role=0.3, pattern_confidence=0.2,
                                      mean_crest_factor=5.0, object_density=3.0),
                description="sparse, sustained-timbre-dominant, low percussive density"),
        ]

    def build_style_vector(self, t: float) -> StyleVector:
        live = [o for o in self.world.objects.values() if o.status != ObjectStatus.ARCHIVED]
        g = self.world.groove
        n = max(len(live), 1)
        pct_percussive = sum(1 for o in live if o.timbre_class == TimbreLabel.PERCUSSIVE.value) / n
        pct_sustained = sum(1 for o in live if o.timbre_class == TimbreLabel.SUSTAINED.value) / n

        def mean_role(name: str) -> float:
            return float(np.mean([getattr(o.role_scores, name) for o in live])) if live else 0.0

        timing_tightness = float(np.clip(1.0 - g.timing_deviation_ms / 100.0, 0.0, 1.0))
        crest_vals = [o.physical_signature.crest_factor for o in live]
        pattern_conf = self.world.current_pattern.confidence if self.world.current_pattern else 0.0
        return StyleVector(
            tempo_bpm=g.tempo_bpm, tempo_confidence=g.tempo_confidence, syncopation=g.syncopation_index,
            swing=g.swing_ratio, regularity=g.regularity, event_density=g.event_density,
            timing_tightness=timing_tightness, pct_percussive=pct_percussive, pct_sustained=pct_sustained,
            mean_kick_role=mean_role("kick"), mean_bass_role=mean_role("bass"),
            mean_percussion_role=mean_role("percussion"), mean_harmonic_pad_role=mean_role("harmonic_pad"),
            mean_lead_melody_role=mean_role("lead_melody"), pattern_confidence=pattern_conf,
            mean_crest_factor=float(np.mean(crest_vals)) if crest_vals else 0.0,
            object_density=float(len(live)), last_updated=t)

    def nearest_regions(self, style: StyleVector, k: int = 3) -> List[Tuple[str, float, str]]:
        vec = style.as_vector()
        results = []
        for region in self.regions:
            dist = float(np.linalg.norm(vec - region.reference_vector))
            similarity = float(np.clip(1.0 - dist / self.REGION_SIMILARITY_SCALE, 0.0, 1.0))
            results.append((region.name, similarity, region.description))
    ...
    def update(self, t: float) -> None:
        if (t - self._last_update_t) < self.UPDATE_INTERVAL_S:
            return
        self._last_update_t = t
        if self.world.groove.tempo_confidence <= 0:
            return  # no rhythmic evidence yet -- nothing meaningful to characterize
        sv = self.build_style_vector(t)
        self.world.update_style(sv, t)

        nearest = self.nearest_regions(sv)
        if nearest and nearest[0][1] >= self.REGION_REPORT_THRESHOLD:
            top_name, top_sim, _ = nearest[0]
            if top_name != self._prev_top_region:
                self._prev_top_region = top_name
                self.world.emit_event("style_region_shift", None,
                                       {"region": top_name, "similarity": top_sim}, top_sim, t)
        elif self._prev_top_region is not None:
            self._prev_top_region = None


# =============================================================================
# 16. PREDICTION MODEL — real trajectory-based prediction (Sec 10.4)
# =============================================================================

class PredictionModel:
    """Replaces the naive last-state-hold used since the tracker's initial
    build. Sec 10.4 lists what prediction should cover: expected time,
    frequency trajectory, energy trajectory, rhythmic recurrence, envelope
    continuation, relationship continuation. This implements the first
    four for real:

    - frequency trajectory: local linear trend fit over the object's own
      recent GENUINELY-matched history (frozen hold-over values from
      HIDDEN periods are filtered out, not fit), not a flat hold.
    - energy/envelope trajectory: local linear trend in LOG-energy space
      (decay/growth is typically exponential, so it's linear in log
      space), not a flat hold.
    - rhythmic recurrence: if the object's own recent match timing is well
      aligned with the groove's beat grid (using the same beat_alignment
      shared with SemanticRoleEngine), the predicted next-appearance time
      is anchored to the next beat instead of a flat prediction_horizon.

    Relationship continuation is a documented follow-up, not built here --
    real additional scope (would need per-relationship-type continuation
    rules), not a quick addition, the same honest call made for
    SAME_SOURCE matching earlier.
    """

    MIN_POINTS_FOR_TREND = 4          # DERIVED: need at least a few real matches to fit a trend
    TREND_LOOKBACK_POINTS = 10        # DERIVED: bounded recency window for the trend fit
    HISTORY_LOOKBACK_HOPS = 64        # DERIVED: bounded scan window into an object's own history
    RHYTHM_ANCHOR_THRESHOLD = 0.6     # DERIVED: how well-aligned to call an object "rhythmically anchored"
    ...
    def predict(self, obj: SoundObject, t: float) -> Prediction:
        horizon_s = self.p.prediction_horizon_ms / 1000.0
        freq_points, energy_points = self._distinct_recent_points(obj)

        expected_freq = obj.physical_signature.frequency_hz
        basis_parts = ["last-state hold"]
        if len(freq_points) >= self.MIN_POINTS_FOR_TREND:
            times = np.array([p[0] for p in freq_points])
            freqs = np.array([p[1] for p in freq_points])
            slope, _ = np.polyfit(times - times[-1], freqs, 1)
            expected_freq = float(np.clip(freqs[-1] + slope * horizon_s, 1.0, 24000.0))
            basis_parts = [f"frequency trend ({slope:+.1f}Hz/s over {len(freq_points)} points)"]

        expected_energy = obj.physical_signature.envelope
        if len(energy_points) >= self.MIN_POINTS_FOR_TREND:
            times = np.array([p[0] for p in energy_points])
            log_e = np.log(np.maximum(np.array([p[1] for p in energy_points]), 1e-9))
            slope, _ = np.polyfit(times - times[-1], log_e, 1)
            expected_energy = float(np.clip(math.exp(log_e[-1] + slope * horizon_s), 0.0, 1.0))
            basis_parts.append(f"log-energy trend ({slope:+.2f}/s)")

        expected_time = t + horizon_s
        alignment = beat_alignment(self.world.groove, obj.last_observed_time)
        if (1.0 - alignment) >= self.RHYTHM_ANCHOR_THRESHOLD and self.world.groove.next_beat_time:
            expected_time = self.world.groove.next_beat_time
            basis_parts.append(f"rhythmically anchored to next beat (alignment={1.0 - alignment:.2f})")

        return Prediction(target_id=obj.object_id, made_at=t, expected_time=expected_time,
                           expected_frequency_hz=expected_freq, expected_energy=expected_energy,
                           confidence=obj.confidence.prediction, basis="; ".join(basis_parts))

    def _distinct_recent_points(self, obj: SoundObject) -> Tuple[List[Tuple[float, float]],
                                                                   List[Tuple[float, float]]]:
        """Extracts genuinely-matched points from an object's history by
        filtering out frozen hold-over values (a HIDDEN object's history
        repeats its last real reading every hop; filtering consecutive
        duplicates recovers just the real match points)."""
        hist = obj.history[-self.HISTORY_LOOKBACK_HOPS:]
        freq_points: List[Tuple[float, float]] = []
        energy_points: List[Tuple[float, float]] = []
        prev_f, prev_e = None, None
        for h in hist:
            if h.frequency_hz != prev_f:
                freq_points.append((h.time, h.frequency_hz))
                prev_f = h.frequency_hz
            if h.energy != prev_e:
                energy_points.append((h.time, h.energy))
                prev_e = h.energy
        return freq_points[-self.TREND_LOOKBACK_POINTS:], energy_points[-self.TREND_LOOKBACK_POINTS:]


# =============================================================================
# 17. TIMBRE FINGERPRINT + SAME-SOURCE MATCHING (Sec 17, lightweight version)
# =============================================================================

class TimbreFingerprintEngine:
    """Accumulates each object's TimbreFingerprint from the classical
    MFCC-style cepstral construction (log bark-band energy -> DCT), reusing
    the bark filterbank already built for masking. Samples only on hops
    where an object was just confidently matched (same 'just_matched' test
    used elsewhere), not every hop -- both for efficiency and because those
    are the moments its own signature was locally clearest."""

    N_COEFFICIENTS = 10   # DERIVED: low-order cepstral coefficients capture envelope shape
    EMA_ALPHA = 0.1        # DERIVED: running-average smoothing per new sample

    def __init__(self, params: Parameters, world: WorldState, n_bark_bands: int):
        self.p = params
        self.world = world
        self.logger = logging.getLogger("TimbreFingerprintEngine")
        self._dct_basis = self._build_dct_basis(n_bark_bands, self.N_COEFFICIENTS)

    @staticmethod
    def _build_dct_basis(n_bands: int, n_coeff: int) -> np.ndarray:
        """Type-II DCT basis, EXCLUDING the k=0 (DC / overall log-energy)
        term -- standard MFCC practice. c0 reflects loudness, not spectral
        shape; its large magnitude would otherwise dominate cosine
        similarity and mask the shape-discriminating coefficients."""
        basis = np.zeros((n_coeff, n_bands))
        for i, k in enumerate(range(1, n_coeff + 1)):
            basis[i] = np.cos(np.pi * k / n_bands * (np.arange(n_bands) + 0.5))
        return basis

    def update(self, ev: PhysicalEvidence, t: float) -> None:
        log_bark = np.log(np.maximum(ev.bark_energy, 1e-10))
        cepstrum = self._dct_basis @ log_bark

        for obj in self.world.objects.values():
            if obj.status == ObjectStatus.ARCHIVED or abs(obj.last_observed_time - t) >= 1e-6:
                continue
            fp = obj.timbre_fingerprint
            if fp.n_samples == 0:
                new_coeffs = cepstrum.tolist()
            else:
                old = np.array(fp.coefficients)
                new_coeffs = (old + self.EMA_ALPHA * (cepstrum - old)).tolist()
            self.world.apply_update(UpdateRecord(t, "TimbreFingerprintEngine", obj.object_id,
                                                  "timbre_fingerprint.coefficients",
                                                  fp.coefficients, new_coeffs,
                                                  "EMA of cepstral coefficients at confidently-matched hops",
                                                  1.0))
            self.world.apply_update(UpdateRecord(t, "TimbreFingerprintEngine", obj.object_id,
                                                  "timbre_fingerprint.n_samples",
                                                  fp.n_samples, fp.n_samples + 1, "sample count", 1.0))

    @staticmethod
    def similarity(fp_a: TimbreFingerprint, fp_b: TimbreFingerprint) -> float:
        if (fp_a.n_samples < SameSourceEngine.MIN_SAMPLES_FOR_MATCHING
                or fp_b.n_samples < SameSourceEngine.MIN_SAMPLES_FOR_MATCHING):
            return 0.0
        a, b = np.array(fp_a.coefficients), np.array(fp_b.coefficients)
        na, nb = float(np.linalg.norm(a)), float(np.linalg.norm(b))
        if na < 1e-9 or nb < 1e-9:
            return 0.0
        cos_sim = float(np.dot(a, b) / (na * nb))
        return float(np.clip((cos_sim + 1.0) / 2.0, 0.0, 1.0))  # map [-1,1] -> [0,1]


class SameSourceEngine:
    """Cross-object re-identification: 'have I heard this specific timbre
    before, recently, in this track' -- NOT closed-set instrument
    classification (that's SemanticRoleEngine's job). No training, no
    external dataset -- pure cosine similarity over the cepstral
    TimbreFingerprint each object accumulates independently. Focuses on
    re-identifying a PAST (archived/quiet) object against a new one, since
    two objects both currently sounding are trivially different sources by
    construction. Runs on a slow cadence -- fingerprints change slowly."""

    UPDATE_INTERVAL_S = 1.0
    SIMILARITY_THRESHOLD = 0.85       # DERIVED: cosine similarity bar for a SAME_SOURCE claim
    MIN_SAMPLES_FOR_MATCHING = 5      # DERIVED: need a few samples before trusting a fingerprint
    MAX_OCTAVE_SPAN = 4.0              # DERIVED: no single real instrument spans this much register;
                                        # beyond it, raw un-aligned spectral-shape comparison becomes
                                        # unreliable (quiet/noise-floor regions can dominate misleadingly)

    def __init__(self, params: Parameters, world: WorldState):
        self.p = params
        self.world = world
        self.logger = logging.getLogger("SameSourceEngine")
        self._last_update_t = -1e9
    ...
    def update(self, t: float) -> None:
        if (t - self._last_update_t) < self.UPDATE_INTERVAL_S:
            return
        self._last_update_t = t
        candidates = [o for o in self.world.objects.values()
                      if o.timbre_fingerprint.n_samples >= self.MIN_SAMPLES_FOR_MATCHING]
        for a, b in itertools.combinations(candidates, 2):
            if a.status == ObjectStatus.ACTIVE and b.status == ObjectStatus.ACTIVE:
                continue  # both currently sounding -- trivially different simultaneous sources
            fa, fb = a.physical_signature.frequency_hz, b.physical_signature.frequency_hz
            if fa > 0 and fb > 0 and abs(math.log2(fa / fb)) > self.MAX_OCTAVE_SPAN:
                continue
            sim = TimbreFingerprintEngine.similarity(a.timbre_fingerprint, b.timbre_fingerprint)
            if sim >= self.SIMILARITY_THRESHOLD:
                self.world.upsert_relationship(a.object_id, b.object_id, RelationshipType.SAME_SOURCE,
                                                strength=sim, confidence=sim, t=t)


# =============================================================================
# 18. STEM SEPARATION ENGINE — HTDemucs auxiliary cue (Sec 17, Dependency Policy)
# =============================================================================

class StemSeparationEngine:
    """Optional, NON-authoritative evidence from HTDemucs (Hybrid
    Transformer Demucs, v4 -- NOT the older base/hybrid Demucs bundled
    with torchaudio's HDEMUCS_HIGH_MUSDB_PLUS, which is Defossez 2021 v3).
    Uses the standalone `demucs` package specifically to get genuine
    HTDemucs: `pip install demucs`.

    Runs OFFLINE, once, on the whole file -- source separation models are
    not causal/streaming-capable (a dedicated low-latency separator, HS-
    TasNet, exists for that at a real quality cost: ~4.65-5.55 dB SDR vs
    HTDemucs's ~9.0 dB). This fits the existing architecture already:
    analyze_full() already precomputes the whole file up front too.

    This NEVER replaces or overrides the mix-domain PhysicalAnalyzer
    pipeline, which stays the sole source of physical evidence. Per-stem
    bark-band energy is exposed only as one additional weighted cue in
    ObjectTracker.component_scores -- Sec 17's own framing for embeddings
    applies here too: 'evidence for identity/similarity... not the
    identity itself.' Trust is weighted per real published numbers, not
    uniformly: bass/drums get real trust (HTDemucs beats oracle mask
    separation on exactly these two, per the MoisesDB benchmark); 'other'
    -- where pads/synths live -- gets much less, since it's HTDemucs's
    weakest category by a wide margin.

    HONEST CAVEAT: unlike every other module in this file, this one could
    not be exercised end-to-end in the sandbox this file was written in --
    no GPU, and the `demucs` package isn't installed there. Written
    carefully against the current confirmed demucs API (verified via
    documentation, not run), but you are the first real test of this
    specific code path. The graceful-fallback path (demucs unavailable)
    IS tested -- the rest of the pipeline runs identically either way.
    """

    STEM_TRUST = {"drums": 0.9, "bass": 0.9, "vocals": 0.6, "other": 0.35}  # DERIVED, see docstring
    MODEL_NAME = "htdemucs"  # single model, not the slower 4x 'htdemucs_ft' bag-of-models variant

    def __init__(self, params: Parameters):
        self.p = params
        self.logger = logging.getLogger("StemSeparationEngine")
        ...
        try:
            ...
            self._apply_model = _apply_model
            self._device = "cuda" if _torch.cuda.is_available() else "cpu"
            self.available = True
        except ImportError:
            self.logger.warning("demucs package not installed -- stem-affinity cue disabled "
                                 "(rest of the pipeline is unaffected). Install with: pip install demucs")

    def _load_model(self):
        if self._model is None:
            self.logger.info("Loading HTDemucs ('%s') on %s ...", self.MODEL_NAME, self._device)
            self._model = self._get_model(self.MODEL_NAME)
            self._model.to(self._device)
            self._model.eval()
        return self._model

    def separate(self, audio: np.ndarray, sample_rate: int) -> Optional[Dict[str, np.ndarray]]:
        """Returns {stem_name: mono_waveform_at_pipeline_sample_rate} or
        None if unavailable or separation failed for any reason -- callers
        must treat None as 'no stem cue this run', never as an error to
        propagate, consistent with this being auxiliary, non-authoritative
        evidence."""
        ...
        try:
            ...
            if wav.dim() == 1:
                wav = wav.unsqueeze(0).repeat(2, 1)  # HTDemucs expects stereo input
            ...
            if model_sr != sample_rate:
                try:
                    ...
                    wav_np = resample_poly(wav.numpy(), model_sr, sample_rate, axis=-1)
                    wav = torch.from_numpy(np.ascontiguousarray(wav_np, dtype=np.float32))
            ref = wav.mean(0)
            wav_norm = (wav - ref.mean()) / (ref.std() + 1e-8)
            with torch.no_grad():
                sources = self._apply_model(model, wav_norm[None], device=self._device,
                                             split=True, progress=False)[0]
            sources = sources * ref.std() + ref.mean()
            names = list(getattr(model, "sources", ["drums", "bass", "other", "vocals"]))
            stems: Dict[str, np.ndarray] = {}
            for i, name in enumerate(names):
                mono = sources[i].mean(dim=0).cpu().numpy().astype(np.float32)
                if model_sr != sample_rate:
                    mono = resample_poly(mono, sample_rate, model_sr).astype(np.float32)
                stems[name] = mono
            self.logger.info("HTDemucs separation complete: %s", list(stems.keys()))
            return stems
        except Exception as e:  # noqa: BLE001 -- auxiliary cue, must never crash the run
            self.logger.warning("Stem separation failed (%s); continuing without the stem-affinity "
                                 "cue for this run.", e)
            return None


# =============================================================================
# 19. AUDITORY WORLD MODEL — top-level coordinator (Sec 31 canonical loop)
# =============================================================================
...
class AuditoryWorldModel:
    ...
    def __init__(self, params: Optional[Parameters] = None):
        ...
        self.prediction_model = PredictionModel(self.p, self.world)
        self.tracker = ObjectTracker(self.p, self.world, self.prediction_model)
        ...
        self.style = StyleEngine(self.p, self.world)
        self.fingerprint = TimbreFingerprintEngine(self.p, self.world, self.physical.n_bark)
        self.same_source = SameSourceEngine(self.p, self.world)
        self.stem_separation = StemSeparationEngine(self.p)
        self.logger = logging.getLogger("AuditoryWorldModel")
        self.evidence_log: List[PhysicalEvidence] = []  # physical-layer replay support
        self._low_band_mask = self.physical.bark_centers_hz < 250.0  # for phase-lock bass-weighting

    def run(self, audio: np.ndarray, use_stem_separation: bool = True) -> None:
        ...
        stem_bark_energy: Optional[Dict[str, np.ndarray]] = None
        if use_stem_separation and self.stem_separation.available:
            stems = self.stem_separation.separate(audio, self.p.sample_rate)
            if stems:
                stem_bark_energy = {name: self.physical.bark_energy_only(wav) for name, wav in stems.items()}
        ...
        for ev in evidence_list:
            ...
            observations = self.obs_gen.generate(ev, stem_bark_energy)
            ...
            for obs in observations:
                if obs.obs_type == ObservationType.TRANSIENT:
                    ...
                    bass_weight = float(ev.bark_energy[self._low_band_mask].sum() / (ev.bark_energy.sum() + 1e-9))
                    self.rhythm.observe_onset(t, strength, bass_weight)
            ...
            self.style.update(t)                          # style-space vector (self-throttled, slowest loop)
            self.fingerprint.update(ev, t)                 # cepstral timbre fingerprint accumulation
            self.same_source.update(t)                     # cross-object re-identification (self-throttled)
    ...
    def print_style_summary(self) -> None:
        """Prints the style vector (Sec 16) and, only as an honestly-framed
        overlay, similarity to a small set of illustrative reference
        regions -- the vector itself is the real output, not any label."""
        print("\n" + "=" * 78)
        print("STYLE (continuous behavior vector -- Sec 16, not a genre classifier)")
        print("=" * 78)
        sv = self.world.style
        if sv.tempo_confidence <= 0:
            print("  (Not enough rhythmic evidence accumulated yet to characterize style.)")
            return
        print(f"  tempo={sv.tempo_bpm:.1f}bpm  syncopation={sv.syncopation:.2f}  swing={sv.swing:.2f}  "
              f"regularity={sv.regularity:.2f}  event_density={sv.event_density:.2f}/s")
        print(f"  timing_tightness={sv.timing_tightness:.2f}  pct_percussive={sv.pct_percussive:.2f}  "
              f"pct_sustained={sv.pct_sustained:.2f}  pattern_confidence={sv.pattern_confidence:.2f}")
        print(f"  mean role scores: kick={sv.mean_kick_role:.2f} bass={sv.mean_bass_role:.2f} "
              f"percussion={sv.mean_percussion_role:.2f} harmonic_pad={sv.mean_harmonic_pad_role:.2f} "
              f"lead_melody={sv.mean_lead_melody_role:.2f}")
        print(f"\n  Nearest illustrative reference regions (hand-specified, NOT fit to any dataset --")
        print(f"  a low similarity everywhere is a valid, expected outcome, not a failure):")
        for name, sim, desc in self.style.nearest_regions(sv):
            print(f"    {name:<38} similarity={sim:.2f}   ({desc})")
    ...
    def plot_activity(self, save_path: Optional[str] = None) -> None:
        objs_all = sorted(self.world.objects.values(), key=lambda o: o.creation_time)
        if not objs_all:
            print("No objects to plot.")
            return
        status_rank = {"ACTIVE": 0, "RECOVERING": 1, "TENTATIVE": 2, "HIDDEN": 3, "ARCHIVED": 4}
        status_colors = {"TENTATIVE": "#f0ad4e", "ACTIVE": "#5cb85c", "HIDDEN": "#d9534f",
                          "RECOVERING": "#5bc0de", "ARCHIVED": "#777777"}

        # --- collapse confirmed-layer members into one merged row per layer ---
        confirmed_layers = [l for l in self.world.layers.values()
                             if l.status == "CONFIRMED" and len(l.member_ids) >= 2]
        grouped_ids: set = set()
        for layer in confirmed_layers:
            grouped_ids.update(layer.member_ids)

        rows: List[Tuple[float, str, List[Tuple[float, str]]]] = []
        for layer in confirmed_layers:
            members = [self.world.objects[oid] for oid in layer.member_ids if oid in self.world.objects]
            if not members:
                continue
            # merge by timestamp: best (most-alive) status among whichever members have
            # a reading at that time -- members can have staggered creation times.
            time_status: Dict[float, str] = {}
            for m in members:
                for h in m.history:
                    cur = time_status.get(h.time)
                    if cur is None or status_rank[h.status] < status_rank[cur]:
                        time_status[h.time] = h.status
            merged = sorted(time_status.items())
            if not merged:
                continue
            freqs = [m.physical_signature.frequency_hz for m in members if m.physical_signature.frequency_hz > 0]
            freq_desc = f"{min(freqs):.0f}-{max(freqs):.0f}Hz" if freqs else "?"
            label = f"{layer.layer_id} [{len(members)} objs, {freq_desc}]"
            rows.append((merged[0][0], label, merged))

        for obj in objs_all:
            if obj.object_id in grouped_ids or not obj.history:
                continue
            merged = [(h.time, h.status) for h in obj.history]
            label = f"{obj.object_id} ({obj.physical_signature.frequency_hz:.0f}Hz)"
            rows.append((obj.creation_time, label, merged))
        ...
        colors: List[str] = []
        for idx, (_, label, merged) in enumerate(rows):
            run_start_time, run_status = merged[0]
            for i in range(1, len(merged)):
                tt, st = merged[i]
                if st != run_status:
                    segments.append(((run_start_time, idx), (tt, idx)))
                    colors.append(status_colors.get(run_status, "black"))
                    run_start_time, run_status = tt, st
            segments.append(((run_start_time, idx), (merged[-1][0], idx)))
            colors.append(status_colors.get(run_status, "black"))
            ax1.text(merged[0][0] - 0.03, idx, label, va="center", ha="right", fontsize=7.5)

        lc = LineCollection(segments, colors=colors, linewidths=6, capstyle="butt")
        ax1.add_collection(lc)
        ax1.set_ylim(-1, len(rows))
        max_t = max(r[2][-1][0] for r in rows)
        ax1.set_xlim(-1.0, max_t + 0.5)
        ax1.set_yticks([])
        ax1.set_ylabel("Layers / Sound Objects")
        ax1.set_title("Auditory World Model — Decomposed Structure vs Input Audio")
        ...
        # --- bottom: a real spectrogram of the input audio, not just sparse transient
        # ticks -- gives a direct, readable comparison of what's actually IN the audio
        # against what the system extracted above.
        if self.evidence_log:
            mag = np.array([e.magnitude for e in self.evidence_log]).T  # (n_bins, n_hops)
            spec_db = 20.0 * np.log10(mag + 1e-6)
            vmax = float(spec_db.max())
            times_axis = [e.t for e in self.evidence_log]
            freqs = self.physical.short_freqs
            ax2.imshow(spec_db, aspect="auto", origin="lower", cmap="magma",
                       extent=[times_axis[0], times_axis[-1], freqs[0], freqs[-1]],
                       vmin=vmax - 70.0, vmax=vmax)
            ax2.set_yscale("symlog", linthresh=50.0)
            ax2.set_ylim(30.0, freqs[-1])
            ax2.set_ylabel("Input audio (Hz)")

            transient_times = [e.time for e in self.world.events if e.event_type == "transient_detected"]
            if transient_times:
                marker_y = freqs[-1] * 0.9
                ax2.scatter(transient_times, [marker_y] * len(transient_times),
                            marker="v", color="white", s=14, zorder=5, label="detected transient")
...
# =============================================================================
# 20. DEMONSTRATION / MAIN — Colab-ready entry point
# =============================================================================
